...
public final class CausaInactividad_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    
    public CausaInactividad_Actualizar(Usuario us,String tipoConexion) {
        ...
        try {
            ...
            Logger.getLogger(CausaInactividad_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE CAUSA DE INACTIVIDAD");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga una causa de inactividad", "Advertencia", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                alerta_nombre.setText("El nombre de la causa de inactividad no puede estar vacio");
            }else{
                ...
                try {
                    if(!new ControlDB_CausaInactividad(tipoConexion).validarExistenciaActualizar(Objeto)){
                        int respuesta=new ControlDB_CausaInactividad(tipoConexion).actualizar(Objeto, user);
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo la causa de inactividad de forma exitosa");
                            codigo.setText("");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar la causa de inactividad, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe una causa de inactividad con el mismo nombre, valide los datos ingresados", "Advertencia", JOptionPane.ERROR_MESSAGE);
                    }     
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar la causa de inactividad");
                    Logger.getLogger(CausaInactividad_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(CausaInactividad_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void tabla_Listar(String valorConsulta) throws SQLException{      
        ...
        ArrayList<CausaInactividad> listadoObjetos=new ControlDB_CausaInactividad(tipoConexion).buscar(valorConsulta);
        for(CausaInactividad listadoObjetos1:listadoObjetos){

...
public  class MotivoParada_Registrar extends javax.swing.JPanel {
    private String tipoConexion;
    Usuario user;
    public MotivoParada_Registrar(Usuario us,String tipoConexion) {
        user=us;
        initComponents();    
        ...
        try {
            ...
            Logger.getLogger(MotivoParada_Registrar.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("REGISTRO DE MOTIVO DE PARADA");
    ...
    private void btnRegistrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRegistrarActionPerformed
        if(nombre.getText().equals("")){
            ...
            motivoParada.setDescripcion(nombre.getText());
            //Validamos si selecciono activo o inactivo
            if(estado.getSelectedItem().toString().equalsIgnoreCase("ACTIVO")){
                motivoParada.setEstado("1");
            }else{
                motivoParada.setEstado("0");
            }
            try {
                int respuesta=new ControlDB_MotivoParada(tipoConexion).registrar(motivoParada,user);
                if(respuesta==1){
                    JOptionPane.showMessageDialog(null, "Se registro el motivo de parada de forma exitosa");
                    nombre.setText("");
                    estado.setSelectedIndex(0);
                    tabla_Listar("");
                }else{
                    if(respuesta==0){
                        JOptionPane.showMessageDialog(null, "No se pudo registrar el motivo de parada, valide datos");
                    }
                }
    ...
    public void tabla_Listar(String valorConsulta) throws SQLException{
        ...
        ArrayList<MotivoParada> listado=new ControlDB_MotivoParada(tipoConexion).buscar(valorConsulta);
        for (MotivoParada listado1 : listado) {

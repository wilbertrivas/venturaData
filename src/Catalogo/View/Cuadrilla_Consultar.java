...
public final class Cuadrilla_Consultar extends javax.swing.JPanel {
    ArrayList<Equipo> listado=null;
    DefaultTableModel modeloCliente;
   // Usuario user;<
    private String tipoConexion;
    public Cuadrilla_Consultar(String tipoConexion) {
        initComponents();
        ...
        try {
           generarListadoEquipo();
            ...
            Logger.getLogger(Cuadrilla_Consultar.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        add(InternalFrameSelectorCampos, new org.netbeans.lib.awtextra.AbsoluteConstraints(270, 380, 1130, 570));

        titulo.setFont(new java.awt.Font("Tahoma", 1, 18)); // NOI18N
        titulo.setText("CONSULTA DE CUADRILLAS");
    ...
    public void generarListadoEquipo() throws SQLException{
        ...
        if(todos.isSelected()){
            ...
            if(tipo.isSelected()){
                if(contador==0){
                    sql += " WHERE [eq_cdgo]> 99999 AND [te_desc] LIKE '"+selectTipo.getSelectedItem().toString()+"'";
                    contador++;
                }else{
                    sql += " AND [eq_cdgo]> 99999 AND [te_desc] LIKE '"+selectTipo.getSelectedItem().toString()+"'";
                    contador++;
                }
            }
            if(marca.isSelected()){
                if(contador==0){
                    sql += " WHERE [eq_cdgo]> 99999 AND [eq_marca] LIKE '"+selectMarca.getSelectedItem().toString()+"'";
                    contador++;
                }else{
                    sql += " AND [eq_cdgo]> 99999 AND [eq_marca] LIKE '"+selectMarca.getSelectedItem().toString()+"'";
                    contador++;
                }
            }
            if(estado.isSelected()){
                if(contador==0){
                    if(selectEstado.getSelectedItem().toString().equalsIgnoreCase("ACTIVO")){
                        sql += " WHERE [eq_cdgo]> 99999 AND [eq_estad]=1 ";
                    }else{
                        if(selectEstado.getSelectedItem().toString().equalsIgnoreCase("INACTIVO")){
                            sql += " WHERE [eq_cdgo]> 99999 AND [eq_estad]=0 ";
                        }
                    }   
                    contador++;
                }else{
                    if(selectEstado.getSelectedItem().toString().equalsIgnoreCase("ACTIVO")){
                        sql += " AND [eq_cdgo]> 99999 AND [eq_estad]=1 ";
                    }else{
                        if(selectEstado.getSelectedItem().toString().equalsIgnoreCase("INACTIVO")){
                            sql += " AND [eq_cdgo]> 99999 AND [eq_estad]=0 ";
                        }
                    }   
                    contador++;
                }
            ...
            if(especifico.isSelected()){
                if(contador==0){
                    sql += " WHERE [eq_cdgo]> 99999 AND [eq_desc] LIKE ?";
                    busquedaEspecifica="%"+valorBusquedaEspecifica.getText()+"%";
                    contador++;
                }else{
                    sql += " AND [eq_cdgo]> 99999 AND [eq_desc] LIKE ?";
                    busquedaEspecifica="%"+valorBusquedaEspecifica.getText()+"%";
                    contador++;
                }
            }
    ...
    public void tabla_Listar(String sql, String busquedaEspecifica) throws SQLException{
        ...
        listado=new ControlDB_Equipo(tipoConexion).buscarCuadrillasEnAplicacionInterna(sql,busquedaEspecifica);

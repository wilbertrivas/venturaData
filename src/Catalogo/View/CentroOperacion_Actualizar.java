...
public final class CentroOperacion_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    public CentroOperacion_Actualizar(Usuario us, String tipoConexion) {
        user=us;
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de consultar los centro de operaciones");
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE CENTRO DE OPERACION");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga un centro de operacion", "Advertencia", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                ...
                centroOperacion.setCodigo(Integer.parseInt(codigo.getText()));
                ...
                try {
                    if(!new ControlDB_CentroOperacion(tipoConexion).validarExistenciaActualizar(centroOperacion)){
                        int respuesta=new ControlDB_CentroOperacion(tipoConexion).actualizar(centroOperacion, user);
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo el centro de operacion");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            codigo.setText("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar el centro de operacion, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe un centro de Operacion con el mismo nombre, valide información", "Advertencia",JOptionPane.ERROR_MESSAGE);
                    }
                    
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar el centro de operacion");
                    Logger.getLogger(CentroCostoAuxiliar_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(CentroCostoAuxiliar_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (UnknownHostException ex) {
                    Logger.getLogger(CentroOperacion_Actualizar.class.getName()).log(Level.SEVERE, null, ex);

...
public final class PertenenciaEquipo_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    public PertenenciaEquipo_Actualizar(Usuario us,String tipoConexion) {
        user=us;
        ...
        try {
            ...
            Logger.getLogger(PertenenciaEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE PERTENENCIA DE EQUIPOS");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga una Pertenencia de equipo", "Advertencia", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                alerta_nombre.setText("El nombre de la Pertenencia de equipo no puede estar vacio");
            }else{
                ...
                try {
                    if(!new ControlDB_PertenenciaEquipo(tipoConexion).validarExistenciaActualizar(Objeto)){
                        int respuesta=new ControlDB_PertenenciaEquipo(tipoConexion).actualizar(Objeto, user);
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo la Pertenencia de equipo");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            codigo.setText("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar la Pertenencia de equipo, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe una Pertenencia de equipo con el mismo nombre, valide los datos ingresados", "Advertencia", JOptionPane.ERROR_MESSAGE);
                    }
                    
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar la Pertenencia de equipo");
                    Logger.getLogger(PertenenciaEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(PertenenciaEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void tabla_Listar(String valorConsulta) throws SQLException{      
        ...
        ArrayList<Pertenencia> listadoProcedenciaEquipoR=new ControlDB_PertenenciaEquipo(tipoConexion).buscar(valorConsulta);
        for(Pertenencia listadoProcedenciaEquipo:listadoProcedenciaEquipoR){

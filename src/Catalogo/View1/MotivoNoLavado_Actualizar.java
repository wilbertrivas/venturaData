...
public final class MotivoNoLavado_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    public MotivoNoLavado_Actualizar(Usuario us, String tipoConexion) {
        user=us;
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de consultar los motivos del no lavado de vehículos");
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE MOTIVOS DEL NO LAVADO DE VEHÍCULOS");
        add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 10, 670, 30));
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga un motivo del no lavado de vhículos", "Advertencia", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                alerta_nombre.setText("El nombre del motivo no puede estar vacio");
            }else{
                MotivoNoLavado motivoNoLavado = new MotivoNoLavado();
                motivoNoLavado.setCodigo(Integer.parseInt(codigo.getText()));
                ...
                try {
                    if(!new ControlDB_MotivoNoLavado(tipoConexion).validarExistenciaActualizar(motivoNoLavado)){
                        int respuesta=new ControlDB_MotivoNoLavado(tipoConexion).actualizar(motivoNoLavado, user);
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo el motivo de forma exitosa");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            codigo.setText("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar el motivo, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe un motivo con el mismo nombre, valide información", "Advertencia",JOptionPane.ERROR_MESSAGE);
                    }
                    
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar el motivo");
                    Logger.getLogger(MotivoNoLavado_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {

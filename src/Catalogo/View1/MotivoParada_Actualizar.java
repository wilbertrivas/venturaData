...
public final class MotivoParada_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    public MotivoParada_Actualizar(Usuario us, String tipoConexion) {
        user=us;
        initComponents();
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar de consultar los motivos de parada");
            Logger.getLogger(MotivoParada_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE MOTIVOS DE PARADA");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga un motivo de parada", "Advertencia", JOptionPane.INFORMATION_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                alerta_nombre.setText("El nombre del motivo de parada no puede estar vacio");
            }else{
                ...
                try {
                    if(!new ControlDB_MotivoParada(tipoConexion).validarExistenciaActualizar(Objeto)){
                        int respuesta=new ControlDB_MotivoParada(tipoConexion).actualizar(Objeto, user);
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo el motivo de parada");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            codigo.setText("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar motivo de parada, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe un motivo de parada con el mismo nombre, valide los datos ingresados", "Advertencia", JOptionPane.ERROR_MESSAGE);
                    }
                    
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar el motivo de parada");
                    Logger.getLogger(MotivoParada_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(MotivoParada_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void tabla_Listar(String valorConsulta) throws SQLException{      
        ...
        ArrayList<MotivoParada> listadoProcedenciaEquipoR=new ControlDB_MotivoParada(tipoConexion).buscar(valorConsulta);
        for(MotivoParada listadoProcedenciaEquipo:listadoProcedenciaEquipoR){
            String []registro = new String[3];
            registro[0]=""+listadoProcedenciaEquipo.getCodigo();
            registro[1]=""+listadoProcedenciaEquipo.getDescripcion();
            registro[2]=""+listadoProcedenciaEquipo.getEstado();            

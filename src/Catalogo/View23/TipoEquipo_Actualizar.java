...
public final class TipoEquipo_Actualizar extends javax.swing.JPanel {
    Usuario user;
    private String tipoConexion;
    public TipoEquipo_Actualizar(Usuario us,String tipoConexion) {
        user=us;
        ...
        try {
            ...
            Logger.getLogger(TipoEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ACTUALIZACIÓN DE TIPO DE EQUIPOS");
    ...
    private void btnActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnActualizarActionPerformed
        if(codigo.getText().isEmpty()){
            JOptionPane.showMessageDialog(null, "Error.. Se debe carga un tipo de equipo", "Advertencia", JOptionPane.ERROR_MESSAGE);
        }
        else{
            if(nombre.getText().equals("")){
                alerta_nombre.setText("El nombre del tipo de equipo no puede estar vacio");
            }else{
                ...
                try {
                    if(!new ControlDB_TipoEquipo(tipoConexion).validarExistenciaActualizar(Objeto)){
                        int respuesta = 0;
                        try {
                            respuesta = new ControlDB_TipoEquipo(tipoConexion).actualizar(Objeto, user);
                        ...
                        if(respuesta==1){
                            JOptionPane.showMessageDialog(null, "Se actualizo el tipo de equipo");
                            nombre.setText("");
                            estado.setSelectedIndex(0);
                            tabla_Listar("");
                            codigo.setText("");
                            ...
                            if(respuesta==0){
                                JOptionPane.showMessageDialog(null, "No se pudo actualizar el tipo de equipo, valide los datos ingresados");
                            }
                        }
                    }else{
                        JOptionPane.showMessageDialog(null, "Ya existe un tipo de equipo con el mismo nombre, valide los datos ingresados", "Advertencia", JOptionPane.ERROR_MESSAGE);
                    }
                    
                } catch (FileNotFoundException ex) {
                    JOptionPane.showMessageDialog(null, "Error al registrar el tipo de equipo");
                    Logger.getLogger(TipoEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                } catch (SQLException ex) {
                    Logger.getLogger(TipoEquipo_Actualizar.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    public void tabla_Listar(String valorConsulta) throws SQLException{      
        ...
        ArrayList<TipoEquipo> listadoObjetos=new ControlDB_TipoEquipo(tipoConexion).buscar(valorConsulta);
        for(TipoEquipo listadoObjetos1:listadoObjetos){

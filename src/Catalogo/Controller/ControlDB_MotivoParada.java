...
public class ControlDB_MotivoParada {
        ...
        try{
            ...
            if(!validarPorDescripcion(Objeto)){
                ...
                if(!validarPorCodigo(Objeto)){
                    ...
                    JOptionPane.showMessageDialog(null, "Ya existe un motivo de parada registrado con ese código");
                }
            }else{
                JOptionPane.showMessageDialog(null, "Ya existe un motivo de parada registrado con ese nombre");
            }
        }
    ...
    public ArrayList<MotivoParada> buscar(String valorConsulta) throws SQLException{
       ...
       String DB=control.getBaseDeDatos();
        try{
            ...
            JOptionPane.showMessageDialog(null, "Error al tratar al consultar los motivos de parada");
            sqlException.printStackTrace();
        } 
        control.cerrarConexionBaseDatos();
    ...
    public ArrayList<MotivoParada> buscarActivosSinFinalizado() throws SQLException{
        ArrayList<MotivoParada> listadoObjetos = null;
        conexion= control.ConectarBaseDatos();
        String DB=control.getBaseDeDatos();
        try{
            PreparedStatement query= conexion.prepareStatement("SELECT mpa_cdgo, mpa_desc, CASE WHEN (mpa_estad=1) THEN 'ACTIVO' ELSE 'INACTIVO' END AS mpa_estad FROM ["+DB+"].[dbo].[motivo_parada] WHERE [mpa_estad]=1 AND mpa_cdgo <> 1;");
            ResultSet resultSet= query.executeQuery();
            boolean validator=true;
            while(resultSet.next()){ 
                if(validator){

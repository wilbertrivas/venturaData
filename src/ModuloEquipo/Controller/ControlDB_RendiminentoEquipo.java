...
public class ControlDB_RendiminentoEquipo {
    private Connection conexion=null;
    private  Conexion_DB_costos_vg control=null;  
    private String tipoConexion;
    public ControlDB_RendiminentoEquipo(String tipoConexion) {
        this.tipoConexion= tipoConexion;
        control = new Conexion_DB_costos_vg(tipoConexion);
    }
    
    /*public String  consultarRendimiento (Equipo equipo, String fechaInicio, String fechaFinal){    
        String retorno="";
        ...
        try{
            ...
            PreparedStatement query= conexion.prepareStatement("DECLARE @startdate datetime2 = '"+fechaInicio+"';\n" +
                                ...
                                "DECLARE @codigoEquipo INT ='"+equipo.getCodigo()+"';\n" +
                                ...
                                "		CASE WHEN(@cantidadMinutosOperativo   IS NULL) THEN 0 ELSE @cantidadMinutosOperativo END AS cantidadMinutosOperativo ,\n" +
                                "		CASE WHEN(@cantidadMinutosParada   IS NULL) THEN 0 ELSE @cantidadMinutosParada END AS cantidadMinutosParada ;");
            resultSet= query.executeQuery();
            while(resultSet.next()){ 
                retorno= resultSet.getInt(1)+":"+resultSet.getInt(2)+":"+resultSet.getInt(3);
            }
        }catch (SQLException sqlException) {
            JOptionPane.showMessageDialog(null, "Error al tratar al consultar los rendimientos", "Error", JOptionPane.ERROR_MESSAGE);
            sqlException.printStackTrace();
        } 
        control.cerrarConexionBaseDatos();
        return retorno;
    }*/
    public String  consultarRendimientoEquipo (String equipo, String fechaInicio, String fechaFinal){    
        String retorno="";
        conexion= control.ConectarBaseDatos();
        String DB=control.getBaseDeDatos();
        try{
            ...
            resultSet= query.executeQuery();
            while(resultSet.next()){ 
                retorno= resultSet.getString(1)+":"+resultSet.getString(2)+":"+resultSet.getString(3)+":"+resultSet.getString(4)+":"+resultSet.getString(5)+":"+resultSet.getString(6);
            }
        }catch (SQLException sqlException) {
            JOptionPane.showMessageDialog(null, "Error al tratar al consultar los rendimientos", "Error", JOptionPane.ERROR_MESSAGE);
            sqlException.printStackTrace();

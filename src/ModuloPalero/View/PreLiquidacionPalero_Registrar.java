...
public class PreLiquidacionPalero_Registrar extends javax.swing.JPanel implements ActionListener, TableModelListener {
    Usuario user;
    ...
    ArrayList<String> consolidadoPreliquidacion =null;
    boolean validarNext = true;
    int level = 0;

    public PreLiquidacionPalero_Registrar(Usuario us, String tipoConexion) {
        initComponents();
        user = us;
        ...
        if (configuracionLiquidacion != null) {//validamos que se tenga cargada una configuración de liquidación
            ...
            if (opcion == 0) {//El usuario selección la opción si, que desea generar la liquidación para el periodo seleccionado.
                try {
                    ...
                    Logger.getLogger(PreLiquidacionPalero_Registrar.class.getName()).log(Level.SEVERE, null, ex);
                }
        ...
        if (!validarNext) {
            ...
            if (level == 1) {
                ...
                if (level == 2) {//Vamos a proceder a registrar en la base de datos
                    ...
                    //Almancenamos temportalmente el consolidado
                    try{
                        consolidadoPreliquidacion =null;
                        consolidadoPreliquidacion = new ControlDB_LiquidacionPalero(tipoConexion).consultarPreliquidacion();
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                    ...
                    if (level == 3) {//Vamos a proceder a generar la Preliquidación y posterior generar el archivo en excel
                        ...
                        if(listadoliquidacion != null && consolidadoPreliquidacion != null){
                            try {
                                int result=new ControlDB_LiquidacionPalero(tipoConexion).registrar_preLiquidacion(listadoliquidacion,user,consolidadoPreliquidacion);
                                if(result == 1){
                                    JOptionPane.showMessageDialog(null, "Se generó la liquidación de forma exitosa");
                                    listadoPlantillaArchivoLiquidacion= new ControlDB_LiquidacionPalero(tipoConexion).consultar_PreLiquidacion(configuracionLiquidacionliquidar);
